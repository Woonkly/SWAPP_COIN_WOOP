...
contract testStakeManager2 {
    ...
    /// #sender: account-0
    function checkChangeRewardOp3() public {
        Assert.equal(stm.OwnerExist( msg.sender ), true, 'owner should be acc0 op3');
        
        try stm.changeReward(TestsAccounts.getAccount(1), 10**18,false,3) returns (bool ok) {
            Assert.ok(ok, 'cannot set changeReward token op3');
        } catch Error(string memory reason ) {
            // This is executed in case
            // revert was called inside getData
            // and a reason string was provided.
            ...
            // or there was a failing assertion, division
            // by zero, etc. inside getData.
            Assert.ok(false, 'failed unexpected token op3');
            
        }

        try stm.changeReward(TestsAccounts.getAccount(1), 10**18,true,3) returns (bool ok) {
            Assert.ok(ok, 'cannot set changeReward coin op3');
        } catch Error(string memory reason ) {
            // This is executed in case
            // revert was called inside getData
            // and a reason string was provided.
            ...
            // or there was a failing assertion, division
            // by zero, etc. inside getData.
            Assert.ok(false, 'failed unexpected coin op3');
        ...
        Assert.equal(rewCoin,0,"FAIL to changeReward coin op3");
        Assert.equal(rewtoken,0,"FAIL to changeReward token op3");
    ...
    /// #sender: account-0
    function checkSubstractFromStake() public {
        Assert.equal(stm.OwnerExist( msg.sender ), true, 'owner should be acc0');
        
        try stm.substractFromStake(TestsAccounts.getAccount(1),10**16) returns (uint256 li) {
            Assert.equal(li, 1, 'cannot set substractFromStake');
        } catch Error(string memory reason ) {
            // This is executed in case
            // revert was called inside getData
            // and a reason string was provided.
        ...
        Assert.equal(balance,990000000000000000,"FAIL to substractFromStake");
    ...
    /// #sender: account-0
    function checkRemoveStake() public {
        Assert.equal(stm.OwnerExist( msg.sender ), true, 'owner should be acc0');
        
        try stm.removeStake(TestsAccounts.getAccount(1)) {
        ...
        Assert.equal(stm.getStakeCount(),0,"FAIL to checkRemoveStake");
